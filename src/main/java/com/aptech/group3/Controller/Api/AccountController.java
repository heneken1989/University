...
public class AccountController {
	...
	public ResponseEntity<List<User>> getAllAccounts() {
		List<User> accounts = userService.listAll();
	    ...
	    try {
	        ...
	        if (authentication.isAuthenticated()) {
	            ...
	            if (user != null) {
	                ...
	                loginResultDto.setUser_id(user.getId());
	                loginResultDto.setName(user.getName());
	                loginResultDto.setRole(user.getRole());
	                loginResultDto.setAvatar(user.getAvatar());
	...
	public ResponseEntity<RefreshDto> refreshToken(@RequestBody RefreshRecieveDto recieve) {
		...
		if(checktoken) {
			String accessToken = jwtTokenProvider.generateToken(new CustomUserDetails(tokenService.getUserByToken(token)));
