...
public interface MarkSubjectRepository extends JpaRepository<MarkSubject,Long> {
	    ...
	    @Query("SELECT ms FROM MarkSubject ms JOIN class_subject cs ON ms.subject.id = cs.subject.id")
	    List<MarkSubject> findByUserIdAndClassForSubjectId(@Param("userId") Long userId, @Param("classForSubjectId") Long classForSubjectId);
