...
public class UserServiceImpl implements UserDetailsService, UserService {
	...
	/*
	 * public List<UserDto> getStudentsBySubject(Long subjectId) { List<User>
	 * students = userRepository.findBySubjectId(subjectId); return
	 * modelMapper.map(students, new TypeToken<List<UserDto>>() {}.getType()); }
	 */
	...
	public void exportToExcel(List<User> users, ServletOutputStream outputStream) {
		...
		String[] headers = { "ID", "Email", "Name", "Role", "Address", "Avatar", "Infomation", "Code" };
		...
		for (User user : users) {
			...
			row.createCell(6).setCellValue(user.getInfomation());
			row.createCell(7).setCellValue(user.getCode());
		}
